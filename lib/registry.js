...
var getISOCode = function (language) {
    ...
    var lang = internals_1.lowerSnake(language);
    if (DESIGNER_LOCALES_STORE.value[lang]) {
        return lang;
    }
    shared_1.each(DESIGNER_LOCALES_STORE.value, function (_, key) {
...
var reSortBehaviors = function (target, sources) {
    ...
    shared_1.each(sources, function (item) {
        if (!item)
            return;
        if (!externals_1.isBehaviorHost(item))
            return;
        var Behavior = item.Behavior;
        shared_1.each(Behavior, function (behavior) {
            if (findTargetBehavior(behavior))
                return;
            var name = behavior.name;
            shared_1.each(behavior.extends, function (dep) {
...
var DESIGNER_LANGUAGE_STORE = reactive_1.observable.ref(internals_1.getBrowserLanguage());
var DESIGNER_GlobalRegistry = {
    ...
    setDesignerBehaviors: function (behaviors) {
        DESIGNER_BEHAVIORS_STORE.value = behaviors.reduce(function (buf, behavior) {
            if (externals_1.isBehaviorHost(behavior)) {
                return buf.concat(behavior.Behavior);
            }
            else if (externals_2.isBehaviorList(behavior)) {
    ...
    getDesignerMessage: function (token, locales) {
        ...
        if (!locale) {
            for (var key in DESIGNER_LOCALES_STORE.value) {
                var message = path_1.Path.getIn(DESIGNER_LOCALES_STORE.value[key], internals_1.lowerSnake(token));
                if (message)
                    return message;
            }
            return;
        }
        return path_1.Path.getIn(locale, internals_1.lowerSnake(token));
    },
    ...
    registerDesignerLocales: function () {
        ...
        packages.forEach(function (locales) {
            internals_1.mergeLocales(DESIGNER_LOCALES_STORE.value, locales);
        });
    },
